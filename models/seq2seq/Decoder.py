...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout = 0.2, model_type = "RNN"):
        ...
        if model_type == "RNN":
            self.recurrent_layer = nn.RNN(input_size=self.emb_size, hidden_size=self.decoder_hidden_size,
                                          batch_first=True)
        else:
            self.recurrent_layer = nn.LSTM(input_size=self.emb_size, hidden_size=self.decoder_hidden_size,
                                           batch_first=True)
    ...
    def forward(self, input, hidden):
        ...
        # recurrent
        intermediate_output = self.recurrent_layer(dropped_out, hidden)
        output, hidden = self.recurrent_layer(dropped_out, hidden)
        # linear and log softmax
        linear_out = self.linear_layer(output)
